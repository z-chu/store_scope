...
* The `equatable` constraint is now `^2.1.0` (was `^2.0.7`). Argument providers mix in `Equatable` instead of the deprecated `EquatableMixin`; `Equatable` only became usable as a mixin in 2.1.0. The comparison semantics are unchanged — `EquatableMixin` and `Equatable` have identical `==` / `hashCode` / `toString` implementations.
